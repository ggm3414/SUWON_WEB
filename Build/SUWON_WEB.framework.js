...
function _ConvertEucKrToUnicode(ptr, size) {
 var decoder = new TextDecoder("euc-kr");
 var byteArray = new Uint8Array(Module.HEAPU8.buffer, ptr, size);
 var resultString = decoder.decode(byteArray);
 var bufferSize = lengthBytesUTF8(resultString) + 1;
...
function _DownloadFile(array, size, fileNamePtr) {
 ...
 console.log("unity call DownloadFile fileName = " + fileName);
 console.log("unity call DownloadFile size = " + size);
}
...
function _OnPOISpotSelected(id) {
 try {
  var idStr = UTF8ToString(id);
  if (typeof window.GetPoiInfo === "function") {
   window.GetPoiInfo(idStr);
  } else {
   console.error("OnPOISpotSelected function is not defined.");
  }
 } catch (e) {
  console.error(e);
 }
...
function _SendMessageToWebApp(jsonRoot) {
 try {
  var encodedString = UTF8ToString(jsonRoot);
  if (typeof window.SendMessageToWebApp === "function") {
   window.SendMessageToWebApp(encodedString);
  } else {
   console.error("SendMessageToWebApp function is not defined.");
  }
 } catch (e) {
  console.error(e);
 }
...
function _SetCurrentCamAngle(camAngle) {
 if (typeof window.SetCamAngleInfo === "function") {
  window.SetCamAngleInfo(camAngle);
 } else {
  console.error("SetCamAngleInfo function is not defined.");
 }
}

function _SetCurrentCamHeight(camHeight) {
 if (typeof window.SetCamHeightInfo === "function") {
  window.SetCamHeightInfo(camHeight);
 } else {
  console.error("SetCamHeightInfo function is not defined.");
 }
}

function _SetCurrentCompassAngle(compassAngle) {
 if (typeof window.SetCompassAngleInfo === "function") {
  window.SetCompassAngleInfo(compassAngle);
 } else {
  console.error("SetCompassAngleInfo function is not defined.");
 }
}

function _SetCurrentCoordinates(longitude, latitude) {
 if (typeof window.SetCoordInfo === "function") {
  window.SetCoordInfo(longitude, latitude);
 } else {
  console.error("SetCoordInfo function is not defined.");
 }
}

function _SetFloodingSimulationHeightValues(current, max, min) {
 try {
  if (typeof window.SetFloodingSimulationHeightValues === "function") {
   window.SetFloodingSimulationHeightValues(current, max, min);
  } else {
   console.error("SetFloodingSimulationData function is not defined.");
  }
 } catch (e) {
  console.error(e);
 }
...
function _doneJob(done) {
 try {
  if (typeof window.doneJob === "function") {
   window.doneJob(done);
  } else {
   console.error("doneJob function is not defined.");
  }
 } catch (e) {
  console.error(e);
 }
...
function _setPointDataSearchObject(jsonPoints) {
 console.log("unity call setPointDataSearchObject");
 try {
  var encodedString = UTF8ToString(jsonPoints);
  if (typeof window.fnUserSearchOnMap === "function") {
   window.fnUserSearchOnMap(encodedString);
  } else {
   console.error("fnUserSearchOnMap function is not defined.");
  }
 } catch (e) {
  console.error(e);
 }
...
var asmLibraryArg = {
 "ConvertEucKrToUnicode": _ConvertEucKrToUnicode,
 ...
 "OnPOISpotSelected": _OnPOISpotSelected,
 "SendMessageToWebApp": _SendMessageToWebApp,
 "SetCurrentCamAngle": _SetCurrentCamAngle,
 "SetCurrentCamHeight": _SetCurrentCamHeight,
 "SetCurrentCompassAngle": _SetCurrentCompassAngle,
 "SetCurrentCoordinates": _SetCurrentCoordinates,
 "SetFloodingSimulationHeightValues": _SetFloodingSimulationHeightValues,
 ...
 "invoke_viiffiffii": invoke_viiffiffii,
 ...
 "mktime": _mktime,
 "setPointDataSearchObject": _setPointDataSearchObject,
...
var dynCall_viffiffiffiiii = Module["dynCall_viffiffiffiiii"] = createExportWrapper("dynCall_viffiffiffiiii");
...
var dynCall_vjddiii = Module["dynCall_vjddiii"] = createExportWrapper("dynCall_vjddiii");
...
function invoke_viiffiffii(index, a1, a2, a3, a4, a5, a6, a7, a8, a9) {
 var sp = stackSave();
 try {
  dynCall_viiffiffii(index, a1, a2, a3, a4, a5, a6, a7, a8, a9);
 } catch (e) {
  stackRestore(sp);
  if (e !== e + 0 && e !== "longjmp") throw e;
  _setThrew(1, 0);
